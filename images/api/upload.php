...
*/
    ...
    if($answer['hash'] && $answer['status']=='ok')
    {
        ...
        if(getDeleteCodeOfHash($answer['hash']))
        {
            ...
            $answer['Thumb_delete_code'] = getDeleteCodeOfHash('Thumb'.$answer['hash']);
            $answer['Thumb_delete_url'] = URL.'delete_'.getDeleteCodeOfHash('Thumb'.$answer['hash']).'/Thumb'.$answer['hash'];
        }
            
        storageControllerUpload($answer['hash']);
        storageControllerUpload('Thumb'.$answer['hash']);
    }
