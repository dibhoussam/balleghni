...
class ImageController implements ContentController
{
   ...
   public function createThumbjpeg($destImagePath, $thumbWidth=100 ,$source)
    {

        if(is_dir(ROOT.DS.'data'.DS.$destImagePath) && file_exists(ROOT.DS.'data'.DS.$destImagePath.DS.$destImagePath)) return;
        mkdir(ROOT.DS.'data'.DS.$destImagePath);
        $file = ROOT.DS.'data'.DS.$destImagePath.DS.$destImagePath;

        addSha1($destImagePath,sha1_file($file));
        //creating a delete code
        $deletecode = getRandomString(32);
        $fh = fopen(ROOT.DS.'data'.DS.$destImagePath.DS.'deletecode', 'w');
    ...
    public function handleUpload($tmpfile,$hash=false)
    {
        ...
        if(is_dir(ROOT.DS.'data'.DS.'Thumb'.$hash) ) {
        //    return;
        
        } else {
        mkdir(ROOT.DS.'data'.DS.'Thumb'.$hash);
        //$file = ROOT.DS.'data'.DS.'Thumb'.$hash.DDS.'Thumb'.$hash;
}
        ...
        $tmp = storeFile($tmpfile,$hash,true);
        $tmp2 =ROOT.DS.'data'.DS.$hash.DS.$hash ;
        $img = new Zebra_Image();
        $img->source_path =ROOT.DS.'data'.DS.$hash.DS.$hash;
        $img->target_path =ROOT.DS.'data'.DS.'Thumb'.$hash.DS.'Thumb'.$hash; 
        $img->sharpen_images = true;
        $img->resize(150, 150, ZEBRA_IMAGE_CROP_CENTER);
           //creating a delete code
         $deletecode = getRandomString(32);
         $fh = fopen(ROOT.DS.'data'.DS.'Thumb'.$hash.DS.'deletecode', 'w');
	     fwrite($fh, $deletecode);
	     fclose($fh);
      // createThumbjpeg("Thumb".$hash  , $thumbWidth=100 ,$tmp2);
       // createThumb( Thumb.$hash, $thumbWidth=100 ,$source);
        return array('status'=>'ok' ,'hash'=>$hash,'url'=>URL.$hash , 'urlThumb'=>URL.'Thumb'.$hash  );
...
/*
    public function createThumbpng($destImagePath, $thumbWidth=100 ,$source)
    {
